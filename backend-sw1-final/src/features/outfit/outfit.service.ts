...
export class OutfitService {
  ...
  async findOne(id: string) {
    const outfit = await this.prisma.outfit.findUnique({ where: { id }, include: OUTFIT_INCLUDE });
    if (!outfit) throw new NotFoundException(`Outfit con ID ${id} no encontrado`);
    return outfit;
  }
  ...
  private async assertOwner(id: string, userId: string) {
    const outfit = await this.prisma.outfit.findUnique({
      where: { id },
      include: { garmentOutfits: { include: { garment: { include: { closet: { select: { userId: true } } } } } } },
    });
    if (!outfit) throw new NotFoundException(`Outfit con ID ${id} no encontrado`);
    const owned = outfit.garmentOutfits.some(go => go.garment.closet.userId === userId);
  ...
  async generateTryOn(outfitId: string, userId: string): Promise<{ tryOnImageUrl: string }> {
    const outfit = await this.prisma.outfit.findUnique({
      where: { id: outfitId },
      include: {
        garmentOutfits: {
          include: { garment: { include: { closet: { select: { userId: true } } } } },
          orderBy: { order: 'asc' },
        },
      },
    });
    if (!outfit) throw new NotFoundException(`Outfit ${outfitId} no encontrado`);
    ...
    try { bodyPhotoUrl = await this.storage.getSignedUrl(userAttr.bodyPhotoPath); } catch { /* ignorar */ }
    ...
    try {
      const imgRes = await fetch(bflUrl, { signal: AbortSignal.timeout(30_000) });
      if (imgRes.ok) {
        const imgBuffer = Buffer.from(await imgRes.arrayBuffer());
        const uploaded = await this.storage.uploadBuffer(imgBuffer, 'try-on.jpg', 'image/jpeg', 'try-on');
        tryOnImageUrl = uploaded.url;
        console.log('[TryOn] Re-subido a Cloudinary:', tryOnImageUrl.slice(0, 60));
      }
    } catch (e) {
      console.warn('[TryOn] No se pudo re-subir a Cloudinary, usando URL temporal:', (e as Error).message);
    }
  ...
  async regenerateTryOn(outfitId: string, userId: string): Promise<{ tryOnImageUrl: string }> {
    await this.prisma.outfit.update({ where: { id: outfitId }, data: { tryOnImageUrl: null } });
  ...
  async createManual(name: string, garmentIds: string[], userId: string) {
    ...
    if (!allOwned) throw new ForbiddenException('Algunas prendas no te pertenecen');
