...
export class HairstyleService {
  ...
  async uploadHairstyles(files: Express.Multer.File[], gender?: string) {
    ...
    for (const file of files) {
      ...
      try {
        const emb = await this.pythonAi.embedHairstyle(
          file.buffer,
          file.mimetype,
        );
        ...
        this.logger.warn(
          `Skipping CLIP embedding for ${file.originalname}: ${(err as Error).message}`,
        );
      }
  ...
  async addFavorite(userId: string, hairstyleId: string) {
    const hairstyle = await this.prisma.hairstyle.findUnique({
  ...
  async removeFavorite(userId: string, hairstyleId: string) {
    await this.prisma.hairstyleFavorite.deleteMany({
      where: { userId, hairstyleId },
    });
    return { success: true };
  }
  ...
  async recommendFromFace(faceFile: Express.Multer.File, userId?: string) {
    ...
    this.logger.log(
      `User gender resolved: ${userGender ?? 'unknown (no filter)'}`,
    );

    // Filter catalog: keep hairstyles matching the user's gender or marked UNISEX / null
    const hairstyles = userGender
      ? allHairstyles.filter(
          (h) => !h.gender || h.gender === 'UNISEX' || h.gender === userGender,
        )
      : allHairstyles;
    ...
    if (hairstyles.length === 0) {
      ...
      this.logger.warn(
        `No hairstyles match gender ${userGender}. Using full catalog.`,
      );
      if (allHairstyles.length) hairstyles.push(...allHairstyles);
    }

    // ── Attempt 1: Python microservice ─────────────────────────────────────────
    const pythonResult = await this._tryPythonRecommendation(
      faceFile,
      hairstyles,
      userGender ?? null,
    );
    if (pythonResult) {
      this.logger.log(
        `[Python] Recommendation succeeded. Face shape: ${pythonResult.faceShape}`,
      );
    ...
    this.logger.warn(
      'Python service unavailable or failed. Falling back to Gemini.',
    );
  ...
  async tryOnHairstyle(
    ...
    try {
      const params = await this.aiService.extractHaircutParams(
        hairstyle.description ?? '',
      );
      haircut = params.haircut;
      hairColor = params.hairColor;
      this.logger.log(
        `[TryOn/Gemini] haircut="${haircut}" color="${hairColor}" gender="${gender}"`,
      );
    } catch (err) {
      this.logger.warn(
        `[TryOn] Gemini extraction failed, using regex: ${(err as Error).message}`,
      );
      haircut = this._mapDescriptionToHaircut(hairstyle.description ?? '');
      hairColor = this._mapDescriptionToColor(hairstyle.description ?? '');
      this.logger.log(
        `[TryOn/Regex] haircut="${haircut}" color="${hairColor}" gender="${gender}"`,
      );
    }

    const replicateUrl = await this._replicateTryOn(
      apiKey,
      base64Image,
      haircut,
      gender,
    ...
    try {
      ...
      this.logger.warn(
        `[TryOn] Re-upload failed, returning Replicate URL: ${(err as Error).message}`,
      );
    }
    ...
    if (!createRes.ok) {
      if (createRes.status === 402) {
        throw new BadRequestException(
          'La cuenta de Replicate no tiene saldo. Recarga en replicate.com/account/billing.',
        );
      }
  ...
  private _mapDescriptionToHaircut(description: string): string {
    ...
    if (
      has('fade', 'degradado') &&
      has('brillante', 'gomina', 'bien peinado', 'liso')
    )
      return 'Slicked Back';
    if (has('fade', 'degradado') && has('volumen', 'cresta'))
      return 'Faux Hawk';
    if (has('mohawk') && has('fade', 'degradado')) return 'Mohawk Fade';
    if (has('rizado', 'curly', 'crespo') && has('largo', 'long'))
      return 'Curly';
    if (has('ondulado', 'wavy') && has('largo', 'long')) return 'Wavy';
    ...
    if (has('slicked back', 'hacia atrás', 'peinado atrás'))
      return 'Slicked Back';
  ...
  private _mapDescriptionToColor(description: string): string {
    ...
    if (/castaño|brunette|marrón|café|chocolate|chestnut/.test(d))
      return 'Chestnut';
    ...
    if (/rubio oscuro|dark blonde|castaño muy claro/.test(d))
      return 'Ash Blonde';
  ...
  private async _resolveUserGender(userId?: string): Promise<string | null> {
    ...
    try {
      ...
      if (
        ['male', 'masculino', 'hombre', 'm', 'masc'].some((v) => g.includes(v))
      )
        return 'MALE';
      if (
        ['female', 'femenino', 'mujer', 'f', 'fem'].some((v) => g.includes(v))
      )
        return 'FEMALE';
      return null;
    } catch {
  ...
  private async _tryPythonRecommendation(
    ...
    try {
      ...
      const catalogWithUrls = sortedHairstyles.map((h) => ({
        ...h,
        imageUrl: h.imageUrl ?? '',
      }));
      ...
      this.logger.warn(
        `Python recommendation path failed: ${(err as Error).message}`,
      );
      return null;
    }
  }
  ...
  private async _geminiRecommendation(
    ...
    const recommended =
      hairstyles.find((h) => h.id === result.hairstyleId) ?? hairstyles[0];
