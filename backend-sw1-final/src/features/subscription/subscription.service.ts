...
export class SubscriptionService {
  ...
  async createCheckout(userId: string, planId: SubscriptionPlan) {
    ...
    if (user.subscription?.stripeSubscriptionId) {
      try {
        ...
        if (existing.status === 'incomplete' && existing.latest_invoice) {
          ...
          const clientSecret = await this.getClientSecretFromInvoice(
        ...
        if (
          ...
          this.logger.warn(
            `Could not retrieve existing sub: ${(e as Error).message}`,
          );
        } else {
          throw e;
        }
    ...
    if (!invoiceId)
      throw new Error('Stripe no generó una invoice para esta suscripción');
  ...
  async handleWebhook(rawBody: Buffer, signature: string) {
    ...
    if (!secret) {
      this.logger.warn(
        'STRIPE_WEBHOOK_SECRET not set — skipping signature check',
      );
    }
  ...
  async getStatus(userId: string) {
    ...
    if (
      ...
      sub.status !== SubscriptionStatus.PREMIUM
    ) {
      try {
        ...
        if (stripeSub.status === 'active' || stripeSub.status === 'trialing') {
          ...
          this.logger.log(
            `Synced PREMIUM from Stripe for userId=${userId} (webhook fallback)`,
          );
          return {
        ...
        this.logger.warn(
          `Could not sync Stripe status for userId=${userId}: ${(e as Error).message}`,
        );
      }
    }
  ...
  private async getClientSecretFromInvoice(invoiceId: string): Promise<string> {
    const list = await (this.stripe.client as any).invoicePayments.list({
      invoice: invoiceId,
    });
    ...
    if (!piId)
      throw new Error('No payment_intent ID in InvoicePayment.payment');
  ...
  private async onPaymentSucceeded(invoice: DahliaInvoice) {
    ...
    if (sub && invoice.amount_paid > 0) {
      ...
      this.logger.log(
        `Payment succeeded for subscription ${subId}, amount=${invoice.amount_paid}`,
      );
    }
  }
